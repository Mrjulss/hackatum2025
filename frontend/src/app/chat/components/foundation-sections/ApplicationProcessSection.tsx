        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {antragsprozess.required_documents.map((doc, idx) => (
              <div
                key={idx}
                className={`bg-white p-4 rounded-xl shadow-sm border-2 transition-all ${doc.required
                    ? 'border-red-200 hover:border-red-300'
                    : 'border-gray-200 hover:border-gray-300'
                  }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${doc.required ? 'bg-red-100' : 'bg-gray-100'
                    }`}>
                    <svg
